...
| R02-20 | 轮播图 | T | ✔5 | `L`＋新主题 `mjy-carousel`；每个选项一张图，图由**平台资产服务**签发取件地址（ADR 0019），版本随发布钉死 | 选项代码 | `""` |
...
| `mjy-carousel` | R02-20 | `L` | `slides`（必填，一张对一个选项）、`autoplay` | `mjy_carousel_slides`（JSON，含 `url`／`alt`／`assetVersion`）、`mjy_carousel_autoplay` |
...
## 三之六、R02-20 轮播图：第一条接上平台资产服务的题型（切片 02.6）

**为什么先挑它。** 卡在资产服务上的七类里，只有它所需的能力被本切片的资产服务**完整覆盖**：
作者上传图、平台留版本、作答者匿名取图。其余六类各缺一件本切片没做的东西——
R02-21 要播放凭据与播放进度取证，R02-25/26 要作答者侧上传（必须走上传会话，见 ADR 0006 限制 3），
R02-32 要服务端图片合成，R02-39/41 要画布快照与按作答者身份的授权（ADR 0019 已知限制 2）。
挑一条能**整条打通**的，胜过把七条各做一半。

**归类从 P 改成 T。** 先前把它记成 P（插件＋副表）是在没有资产服务时按「反正要新东西」估的。
真做下来，需求验收那四项——切换、加载失败、替代文本、媒体版本留存——没有一项需要新的数据形状：
数据就是原生单选，一张幻灯片对一个选项，答案列存选项代码。于是插件一行没动、副表一张没建，
引擎自己的 `checkValidityAnswer` 就是服务端闸门。

**图从哪来。** 作者在草稿里写 `themeOptions.slides[].assetId`；平台发布时把每一处 `assetId`
换成签名取件地址并写上当时的版本号（ADR 0019 决定 5），登记一行引用。网关**只看见 `url`**，
看见 `assetId` 直接 422——那说明平台没有物化，发出去作答页上的图会全裂。

这条链上有三处「作者填不进来」的闸门，值得单独记一笔：

- 作者自带地址：平台判**定义非法**，不发布。挡它靠的是两侧合起来的一条不变式——
  平台侧「草稿里出现 `assetVersion` 即非法」（它只由平台在解析 `assetId` 时写入），
  网关侧「每种带资产的主题都把 `assetVersion` 列为必填」。少任何一条，作者只要不写 `assetId`、
  直接填好 `url` 与 `assetVersion`，改写器就根本不会访问那个对象，资产服务等于白做
  （独立安全审查抓到的一条，见 ADR 0019 审查小节）；
- 网关只认 `http(s)://` 开头的地址，`javascript:`、`data:`、`//` 一律 422。
  这比现存的热力图底图／货架图／PK 参赛图严格得多——那三处至今只校验「非空且不超长」；
- 替代文本 `alt` **必填**。无障碍是 R02-20 的验收项，不是可选装饰。

**媒体版本留存落在哪一刻**：发布时取当前版本写进快照。作者之后上传新版本，已发布的那一版
纹丝不动；要让在线问卷用新图，必须改版再发布（ADR 0012），与问卷的其他改动同一条路、同一份审批。
...
- **插件扩展**（P 类余下 14 条）：副表的「结构版本」列已落地（ADR 0006 限制 9 解除），
  ...
  切片 02.6 交付 R02-20 轮播图（见第三之六节），它是**第一条接上平台资产服务的题型**。
  **仍未做的 6 条各有外部阻塞**：R02-03 需平台字典服务（省市区树数千节点，
  塞不进题目属性）与级联清值策略；R02-21/25/26/32/39/41 共 6 条仍卡在资产服务**尚未做的**
  那几件事上——授权播放的播放凭据（R02-21）、作答者上传（R02-25/26）、图片合成（R02-32）、
  画布快照（R02-39/41）。资产服务本身（上传、版本留存、签名取件）已就位，见 ADR 0019。
  R02-17 的参赛图、货架图与热力图底图**仍是作者填的 URL**：改造它们是独立的一片，
  本切片没有顺手改，也没有扩大这个欠账。
