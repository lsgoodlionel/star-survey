...
- 状态：原型已实现、经独立审查并修复、真实进程故障注入通过；投递中继待完成
- 日期：2026-09-18
...
`platform/deploy/test/run-fault-injection.sh`（`TEST_DB=mysql|pgsql`）：通过 RemoteControl 建一份单题问卷并激活，再用 HTTP 像真实填写者一样填写提交。测试专用插件 `FaultInjector` 只挂载进测试容器，优先级高于桥接插件，在指定事件里对当前 PHP worker 发 SIGKILL。

| 场景 | 中断点 | MariaDB 10.11 | PostgreSQL 16 |
|---|---|---|---|
| 无故障 | — | 通过：钩子记 1 条完成 | 通过 |
| 钩子前崩溃 | `afterSurveyComplete`（`submitdate` 已写入） | 通过：崩溃后无完成事件；两次 cron 后恰好 1 条，来源 scanner；两次扫描耗时 0.26 秒 | 通过（0.36 秒） |
| `submitdate` 前崩溃 | 最后一页的 `afterResponseSave` | 通过：答卷未完成，扫描后也不产生完成事件 | 通过 |

**结论**：完成事件的可靠性不需要核心补丁。最坏情况下的丢失窗口＝cron 间隔＋一次扫描耗时；生产环境 cron 设为每分钟一次。总方案里另外两个中断点（写日志后投递前、平台提交后应答前）属于投递中继和平台 Inbox，届时在各自的契约测试中覆盖。
...
## 修订（2026-09-21）：每个引擎实例使用独立密钥

**问题（高危）**：原先所有引擎共用一个签名密钥，发送方是哪个实例只是请求体里的一个字段。任何持有该密钥的引擎都能冒充其他租户的实例投递事件；私有化与混合云下引擎运行在客户机房，等于客户 A 的引擎可以向客户 B 的数据写事件。

**决定**：
1. 平台只保存一份主密钥（`PLATFORM_ENGINE_EVENTS_SECRET`）；每个实例的密钥由它派生：`hex(HMAC-SHA256(主密钥, "mjy-engine-events/v1/" + 实例 id))`。引擎侧把派生值配置为 `MJY_PLATFORM_EVENTS_SECRET`。PHP 以该十六进制串作 `hash_hmac` 的密钥，签名算法不变。
2. 发送端新增请求头 `X-Mjy-Engine-Instance`；接收端按该头派生密钥验签。实例头不在签名串里也无妨：换成别的实例就要用别的实例的密钥，拿不到。
3. 验签通过后，**批次内每个事件的实例 id 都必须等于已认证实例**，否则整批以 403 `engine_instance_mismatch` 拒收、一行不写——否则实例 X 可以用自己的密钥签一个装着实例 Y 事件的批次。
4. 租户只按已认证实例经实例目录查得；下游只认过滤器写入的请求属性，不读请求头、不信请求体。
5. 运营取密钥：`POST /v1/platform/engine-instances/{instanceId}/event-secret`，仅限平台运营角色，写审计，响应 `Cache-Control: no-store`。

**开通顺序**：实例目录只对"已激活租户的已激活实例"返回归属，因此须先激活租户与实例，再为引擎签发密钥；激活前引擎的事件本就会被拒收、留在引擎侧待补投，不丢事件。

**验证**：Java 全量 328 个测试通过，新增覆盖：正确实例与密钥被接受；实例头与批次内实例不符被拒且不写库；用别的实例的密钥签名被拒；缺少实例头被拒；用主密钥本身签名被拒；派生结果确定且各实例不同；取密钥接口 403/401、返回值与验签端一致、写审计。金标准请求已用仓库里真实的 `MjyEventRelay` 与 `MjyHttpEventTransport` 重新抓取，实例 id 取符合登记规则的 `hd-engine-01`。

**残余风险**：主密钥泄露即可推出所有实例密钥，主密钥必须放在密钥管理服务中；实例密钥轮换需升级派生前缀版本（`v1` → `v2`）并过渡期内双认。
