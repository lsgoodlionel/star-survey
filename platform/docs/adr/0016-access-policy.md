...
- ✅ 离线 CSV 地区数据源已改走**预排序索引＋二分**（`MjyGeoIpIndex`）：首次查询时把 CSV 排序后写成定宽记录的
  索引文件，之后每次查询是 O(log n) 次 `fseek`，与库多大无关；CSV 变新了自动重建，写盘走临时文件 ＋ rename。
  遗留：索引目录不可写时**退回逐行扫描**（答案照旧，只是慢）——把「数据盘只读」变成「所有人都定位不到、
  按 `regionUnknown` 全拒」是个更糟的故障；可用 `MJY_GEOIP_INDEX` 指向一个可写路径避开。
  另有一处口径变化：范围重叠时二分返回起始地址最大的那一条，逐行扫描返回文件里靠前的那一条。
