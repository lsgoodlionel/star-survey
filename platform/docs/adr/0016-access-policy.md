...
- ⏳ 网关 `add_participants` 总是让引擎生成 token（`create_token=true`），定义里写的 token 会被替换；
  ...
  **做这件事时的前置约束**：`PublishResult` 目前不含任何凭据，所以网关的结果存档（按 `requestId` 存档
  完整应答体）里也没有。若把回读来的邀请码放进回执，存档的**回执留存期就等于那批 token 的明文存活期**
  （[ADR 0012](0012-republish-and-drift.md) 决定 7，缺省 7 天）——必须把它降到小时级并在契约里单独写明，
  否则每一个发出去过的邀请码都会在网关磁盘上留一周。墓碑不含正文，不需要第二套规则。
  另一条更省事的路是邀请码根本不进回执：平台自己按 sid 从引擎读回（端到端现在就是这么做的）。
