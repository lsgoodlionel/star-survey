...
- `start` 为匿名端点（限流见增补三）；过期 state 在下次发起时清理（保留 1 天）。
...
- 部门与标签映射（R18-02）；钉钉 Stream 模式、飞书长连接模式未接。
- 企业微信"成员变更"之外的通讯录事件（部门删除等）与飞书 `contact.user.created_v3` 只确认、不处理。

## 增补三（第三波收尾）：匿名端点的进程内限流

事件回调与免登 `start` / `callback` 都是匿名端点（安全审查 MEDIUM）。原计划放在网关 / 入口层，
但那层未必存在，且"验签之前不花钱"这件事只有应用自己知道谁验签通过了，因此在应用内先做一层。

`AnonymousRateLimitFilter`（只用 JDK，无新依赖）挂在两条安全过滤链上，排在授权过滤器之前，
因此判定发生在验签、解密与任何数据库查询之前，也在读请求体之前。

| 决定 | 内容 |
|---|---|
| 1. 两层桶，都按来源地址分 | 「来源地址」（跨连接，挡住换连接号乱撒的扫描）与「来源地址 × 连接」。因为每层都含来源，一个来源的噪声吃不掉另一个来源的额度——**攻击者锁不死某个租户的真实回调** |
| 2. 验签通过的请求退还令牌 | 下游返回 2xx 即说明签名对得上（内容相关的拒绝一律是统一 401），此时把刚扣的两个令牌退回去，改记在该连接的「已验签」额度上。真实回调因此不消耗未验签额度；已验签额度用尽后不再退还，总量仍有界 |
| 3. 免登链不退还 | 谁都能拿到 302，2xx/3xx 不代表来者正当 |
| 4. 来源地址取直连对端 | 刻意不认 `X-Forwarded-For`：没有可信代理清单时它可被伪造，认了等于让攻击者随意换桶（与 ADR 0016 对引擎侧 `MJY_TRUSTED_PROXIES` 的口径一致）。**部署在反向代理后面时须由代理或入口层提供真实地址**，否则所有来源共用一个桶——那时决定 2 仍保证真实回调不受影响 |
| 5. 内存有界 | 每个桶集合按最近使用淘汰（缺省 20000 条）。桶越久没被碰越接近装满，被淘汰的几乎总是满桶，淘汰不会放宽正在生效的限流。连接号不是 UUID 时归入同一个桶，键数不受调用方摆布 |
| 6. 429 不说明原因 | 体只有 `{"error":"rate_limited","message":"too many requests"}` 与 `Retry-After`，不泄露连接是否存在、签名错在哪里；其余失败的统一 401 不受影响 |

配置前缀 `platform.identity.rate-limit`（`enabled`、`per-connection-per-minute` 缺省 60、
`per-source-per-minute` 缺省 300、`verified-per-connection-per-minute` 缺省 600、
`max-buckets`、`retry-after-seconds`）。事件链与免登链各有一套桶，互不相干。

**仍未做**：多实例部署时每个实例各限各的（真实限额 ＝ 缺省值 × 实例数），需要共享计数器或入口层限流；
没有按租户维度的总量限制。
