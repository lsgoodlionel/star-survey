...
- 状态：已实现（WP-01 01.3、01.4），单元／集成测试与真引擎端到端通过
- 日期：2026-09-22
...
### 7. 网关的结果存档留存有界：7 天回执 ＋ 90 天墓碑，过期重放是 410 而不是"没发过"
...
| 墓碑期 | 90 天 | 请求指纹、原状态码、落库时刻、引擎 sid | **410 `result_expired`** |
| 之后 | — | 无 | 当成新请求，会真的再发布一次 |
...
**回执里目前没有凭据。** 邀请码由引擎生成、平台在发布后自行回读（ADR 0016 缺口），
`PublishResult` 里没有 token，所以本决定解决的是无界增长与无用历史数据，不是凭据留存。
若将来把邀请码回读进回执，回执期就等于那批凭据的明文存活期，必须按小时而不是按天设，
并在契约里单独写明——墓碑本身不含正文，不需要第二套规则。
...
- 留存期（决定 7）：网关 `tests/test_store.py`（`RetentionWindowTest` / `PruneSchedulerTest`：窗口内原样、
  ...
  配置下限与两段窗口的先后约束、清理线程首轮不等待且异常不致死）与 `tests/test_service.py`
  （`RetentionTest`：410 的应答形状、过期后不再发布、过期的 422／502 分别报 `surveyId` 空与孤儿 sid、
  指纹不符仍是 400、墓碑到期后真的会再发布一次、410 应答不泄漏引擎口令）；平台
  `HttpPublishGatewayClientTest`（410 归类、无细节的 410 仍是终局）与 `PublishReconciliationTest`
  （过期即终局、孤儿 sid 存档、不再重试）。
...
7. **墓碑期一过，同一 `requestId` 会被当成新请求真的再发布一次**（决定 7）。这是留存有界的代价，
   没有绕过办法：平台侧的人工复核流程必须在墓碑期（缺省 90 天）内收尾。平台目前也没有"人工复核后
   重新驱动这次发布"的接口，人只能直接看引擎、手工清理，再让作者重新发布（换新 requestId）。
8. **留存期约束的是"网关会返回什么"与"表里有什么行"，不是磁盘上的字节**。不跑 `prune-results` 时，
   删掉的行留下的空闲页会被后续写入复用，旧正文在被覆盖前仍躺在文件里；跑了 `VACUUM` 之后，
   WAL 残留、文件系统未擦除的块、以及任何数据库备份仍不在留存期管辖之内。真要按期销毁，
   备份策略得一起管。这也正是"回执里不该放凭据"的另一个理由。
9. **410 这条路径没有真引擎端到端覆盖。** 回执留存期的下限是 24 小时（刻意的防呆），所以端到端脚本
   没法靠"配一个很短的窗口"走到过期；要覆盖只能在脚本里直接改网关 SQLite 的 `created_at` 再重放。
   目前只有网关单元测试与平台测试覆盖（见"验证"）。
10. 平台侧没有把 410 单独建成一个问卷状态：它复用 `publish_failed` ＋ `failed_stage='expired'` ＋
