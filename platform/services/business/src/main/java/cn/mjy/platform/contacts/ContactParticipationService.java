 ...
 *
 * <p>平台不在这里向引擎发码：令牌由发布后从引擎 {@code tokens_<sid>} 读回（ADR 0016 缺口清单），
 * 投放车道把它登记到这里，再按这份映射去发送。本服务只保证两件事：
 * 令牌落在当前在线的已发布版本上；一个令牌同时只属于一个联系人（"续答令牌不互用"）。
 * <b>发送不在本模块。</b>
 */
@Service
public class ContactParticipationService {
    ...
    public ParticipationView link(TenantContext ctx, UUID contactId, UUID surveyId, String participantToken) {
        ...
        return tenantScope.call(ctx.tenantId(), () -> {
            ...
            Optional<ParticipationView> current =
                    participations.findCurrent(contactId, surveyId, version.version());
            if (current.isPresent()) {
                if (current.get().participantToken().equals(token)) {
                    return current.get();
                }
                throw new ContactConflictException("participation_exists",
                        "contact " + contactId + " already holds a token for version " + version.version());
            }
            ParticipationView linked;
            try {
                linked = participations.insert(UUID.randomUUID(), contactId, surveyId, version.version(),
                        version.engineInstanceId(), version.engineSid(), token, ctx.actorId());
            ...
            audit.record(ctx, ContactsAudit.PARTICIPATION_LINK, "contact", contactId,
                    "survey/" + surveyId + "/v" + version.version());
            return linked;
        });
    }
