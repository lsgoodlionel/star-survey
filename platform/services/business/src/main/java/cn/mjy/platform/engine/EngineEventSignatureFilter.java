...
/**
 * /internal/** 的认证过滤器：读出原始请求体（有上限），按 HMAC 验签，通过后以
 * "引擎中继"身份放行，并把同一份字节交给下游解析。
 *
...
final class EngineEventSignatureFilter extends OncePerRequestFilter {
    ...
    private void authenticateAsRelay() {
        SecurityContext context = contexts.createEmptyContext();
        context.setAuthentication(UsernamePasswordAuthenticationToken.authenticated(
                RELAY_PRINCIPAL, null, List.of(new SimpleGrantedAuthority("ROLE_" + RELAY_ROLE))));
