...
**网关分不出「平台签发的地址」与「作者自己填的地址」**——它没有验签的密钥，也不知道平台的对外地址。
真正挡住作者自带地址的是两侧合起来的一条不变式：

* 平台侧：草稿里任何一处出现 ``assetVersion`` 都判定义非法，它只由平台在解析 ``assetId`` 时写入
  （``SurveyAssetSource.requirePlatformKeysUnwritten``）；
* 网关侧：**每一种带资产的主题都必须把 ``assetVersion`` 列为必填**（见下面的 ``_slide``）。

于是作者自带的地址凑不齐一条合法的引用。新增媒体类主题时这两条要一起满足，
少一条就等于把资产服务绕开了（独立安全审查抓到的一条，ADR 0019 审查小节）。
"""
